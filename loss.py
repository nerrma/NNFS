...
class Loss:
    ...
    def remember_trainable(self, trainable_layers):
    ...
    def reg_loss(self, layer):
        ...
        if layer.w_reg_lambda[1] > 0:
            reg_loss += layer.w_reg_lambda[0] * np.sum(layer.weights * layer.weights)

        if layer.b_reg_lambda[1] > 0:
            reg_loss += layer.b_reg_lambda[0] * np.sum(layer.biases * layer.biases)
